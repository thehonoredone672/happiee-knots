...
// --- FETCH PRODUCTS FROM MONGOOSE ATLAS ---
async function initializeProductPage() {
    try {
        ...
        if (data.success && data.products.length > 0) {
            ...
            // Fallback default mock array if your Atlas database collection is empty
            PRODUCTS = [
                { id: '1', name: 'Ruby Luxe Handbag', category: 'Handbags', price: 2499, image: 'https://images.unsplash.com/photo-1590874103328-eac38a683ce7?q=80&w=600', description: 'Handcrafted handbag.' },
                { id: '2', name: 'Sunwoven Citrus Tote', category: 'Handbags', price: 1699, image: 'https://images.unsplash.com/photo-1584916201218-f4242ceb4809?q=80&w=600', description: 'Vibrant handwoven tote.' }
            ];
        }
        
        // Synchronize display collections
        applyFiltersAndSort();
    } catch (err) {
        console.error('Failed to load live database store assets:', err);
        document.getElementById('productsGrid').innerHTML = '<p class="text-muted">Unable to load products right now.</p>';
    }
}
...
function renderProducts(productsToRender) {
    ...
    if (!grid) return; // Guard clause if run on a non-product page
    
    countSpan.textContent = productsToRender.length;

    if (productsToRender.length === 0) {
        grid.innerHTML = `<div class="no-products text-center" style="grid-column: 1/-1; padding: 3rem 0;">
            <p class="text-muted">No products found matching your filter selection.</p>
        </div>`;
        return;
    }
    ...
    grid.innerHTML = productsToRender.map(product => `
        <div class="product-card" data-id="${product._id || product.id}">
            <div class="product-image-wrapper" style="position:relative; overflow:hidden; border-radius:12px; margin-bottom:12px;">
                <img src="${product.image}" alt="${product.name}" class="product-card-img" style="width:100%; height:280px; object-fit:cover; transition: transform 0.3s ease;">
            </div>
            <div class="product-info" style="text-align:left;">
                <span class="product-category" style="font-size:12px; color:#666; text-transform:uppercase; letter-spacing:0.5px;">${product.category}</span>
                <h3 class="heading-font" style="font-size:16px; margin:4px 0; font-weight:600; color:#111;">${product.name}</h3>
                <p class="product-price" style="font-weight:700; color:#d93b7c; margin-bottom:8px;">₹${product.price.toLocaleString('en-IN')}</p>
                <button class="btn btn-primary" onclick="addToCart('${product._id || product.id}')" style="width:100%; padding:10px; font-size:14px; border-radius:50px;">Add to Bag</button>
            </div>
        </div>
    `).join('');
}
...
function applyFiltersAndSort() {
    const searchQuery = document.getElementById('collectionSearch')?.value.toLowerCase() || '';
    const selectedCategory = document.getElementById('categorySelect')?.value || 'all';
    const selectedSort = document.getElementById('sortSelect')?.value || 'newest';
    ...
    } // 'newest' uses default database return order

    renderProducts(filteredProducts);
}

// Bind search and dropdown events
function setupEventListeners() {
    document.getElementById('collectionSearch')?.addEventListener('input', applyFiltersAndSort);
